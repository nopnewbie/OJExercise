...
function<int (int)> func = [value](int i){return i>=value;}; //lambda函数指针
...
int main()
{
    ...
    if(N>1 && A[0]<=A[1])
        func = ([value](int i){return i<value;});//如果是mirror,改变lambda函数
    tree(A,A+N);
    if(res.size()!=N)
        printf("NO\n");
    else
	{//输出
        printf("YES\n");
        printf("%d",res[0]);
		for_each(res.begin()+1, res.end(), [](int n){printf(" %d", n); } ) ;
		printf("\n") ;
	}
    return 0;
}
